          ...
          <div class="feature-shell feature-works">
            ...
            <div class="reel-grid">
              <article v-for="reel in systemReels" :key="reel.title" :class="['system-reel', reel.tone, 'reveal-on-scroll']">
                ...
                <div class="reel-copy"><span>{{ reel.kicker }}</span><h3>{{ reel.title }}</h3><p>{{ reel.description }}</p><small>{{ reel.detail }}</small><button type="button" class="case-study-link" @click="selectedCaseStudy = reel">View case study <span>→</span></button></div>
              </article>
            </div>
          </div>
        </section>
